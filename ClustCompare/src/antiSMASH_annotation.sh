...
usage()
{
	...
	echo "    -o    path to directory where output should be placed"
...
LOG_FILE=${OUTPUT_DIR}/CHANGELOG.txt
...
echo "Ouput will be here: $OUTPUT_DIR"
echo "Updating log: $LOG_FILE"

# Add note to log
echo -e "## `date +%Y-%m-%d:%H:%M:%S` \n\n* antiSMASH_annotations.sh v3.0 started" >> $LOG_FILE 
...
for f in ${$INPUT_DATA_PATH}/*.gbff.gz; do
	[ -e "$f" ] && gunzip $f && echo "unzipping $f"
done

for f in ${$INPUT_DATA_PATH}/*.gbk.gz; do
	[ -e "$f" ] && gunzip $f && echo "unzipping $f"
done

for f in ${$INPUT_DATA_PATH}/*.embl.gz; do
	[ -e "$f" ] && gunzip $f && echo "unzipping $f"
done

for f in ${$INPUT_DATA_PATH}/*.fasta.gz; do
	[ -e "$f" ] && gunzip $f && echo "unzipping $f"
done

for f in ${$INPUT_DATA_PATH}/*.fna.gz; do
	[ -e "$f" ] && gunzip $f && echo "unzipping $f"
done

for f in ${$INPUT_DATA_PATH}/*.fa.gz; do
	[ -e "$f" ] && gunzip $f && echo "unzipping $f"
done
...
for f in ${$INPUT_DATA_PATH}/*.gbff; do
...
for f in ${$INPUT_DATA_PATH}/*.embl; do
	[ -e "$f" ] && ls $f >> genomes.list
done

for f in ${$INPUT_DATA_PATH}/*.fasta; do
	[ -e "$f" ] && ls $f >> genomes.list
done

for f in ${$INPUT_DATA_PATH}/*.fna; do
	[ -e "$f" ] && ls $f >> genomes.list
done

for f in ${$INPUT_DATA_PATH}/*.fa; do
	[ -e "$f" ] && ls $f >> genomes.list
done
...
then
	echo "${OUTPUT_DIR}/Results/ found - moving nodes.tsv there"
else
	echo "Making ${OUTPUT_DIR}/Results - moving nodes.tsv there"
	mkdir ${OUTPUT_DIR}/Results
fi
mv nodes.tsv ${OUTPUT_DIR}/Results

# runs clusters_per_genome.pl on new nodes.tsv file
perl clusters_per_genome.pl -i ${OUTPUT_DIR}/Results/nodes.tsv

# moves clusters_per_genome.pl output files
echo "Moving clusters_per_genome.tsv to ${OUTPUT_DIR}/Results"
mv clusters_per_genome.tsv ${OUTPUT_DIR}/Results
